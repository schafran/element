...
export class SiTimepickerComponent implements ControlValueAccessor, Validator, SiFormItemControl {
  ...
  protected readonly timeControls = this.formBuilder.group({
    hours: this.formBuilder.nonNullable.control('', [this.validateTime.bind(this, 'hours')]),
    minutes: this.formBuilder.nonNullable.control('', [this.validateTime.bind(this, 'minutes')]),
    seconds: this.formBuilder.nonNullable.control('', [this.validateTime.bind(this, 'seconds')]),
    milliseconds: this.formBuilder.nonNullable.control('', [
      this.validateTime.bind(this, 'milliseconds')
    ])
  });
  ...
  protected handleInput(key: keyof Value, value: string): void {
  ...
  protected validateMin(control: AbstractControl): ValidationErrors | null {
    ...
    if (control.value instanceof Date && min) {
      ...
      if (current < min) {
        error.minTime = { actual: current, min: min };
      }
    }
  ...
  protected validateTime(name: string, c: AbstractControl): ValidationErrors | null {
